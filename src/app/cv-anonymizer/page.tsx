...
export default function CVAnonymizer() {
  ...
  const generateAnonymizedWord = async (customData?: StructuredData) => {
    try {
      const dataToUse = customData || structuredData;
      if (!dataToUse) {
        alert('No structured data available. Please process a CV first.');
      ...
      children.push(
        ...
        new Paragraph({
          children: [
            new TextRun({
              text: "Generated by CV Anonymizer",
              italics: true,
              size: 16,
              color: "808080",
            }),
      ...
      if (anonymizedStructuredData.summary) {
        children.push(
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: anonymizedStructuredData.summary,
                size: 22,
              }),
            ],
            spacing: { after: 300 },
      ...
      if (anonymizedStructuredData.work_experience.length > 0) {
        ...
        anonymizedStructuredData.work_experience.forEach((exp) => {
          children.push(
            new Paragraph({
              children: [
                new TextRun({
          ...
          children.push(
            new Paragraph({
              children: [
                ...
                new TextRun({
                  text: anonymizedStructuredData.skills.technical.join(', '),
                  size: 22,
                }),
              ],
              spacing: { after: 100 },
          ...
          children.push(
            new Paragraph({
              children: [
                ...
                new TextRun({
                  text: anonymizedStructuredData.skills.soft.join(', '),
                  size: 22,
                }),
              ],
              spacing: { after: 100 },
          ...
          children.push(
            new Paragraph({
              children: [
                ...
                new TextRun({
                  text: anonymizedStructuredData.skills.languages.join(', '),
                  size: 22,
                }),
              ],
              spacing: { after: 200 },
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `anonymized-cv-${Date.now()}.docx`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
              ...
              <Tabs defaultValue="analysis" className="w-full">
                ...
                <TabsContent value="preview" className="space-y-6 mt-6">
                  <PDFPreview 
                    ...
                    onDownloadWord={(data) => generateAnonymizedWord(data)}
                  />
                </TabsContent>
              </Tabs>
            )}
            ...
            {structuredData && (
              <Card>
                ...
                <CardContent className="space-y-3">
                  <Button 
                    variant="outline" 
                    onClick={() => generateAnonymizedWord()}
                    className="w-full"
                  ...
                  <p className="text-xs text-gray-500 text-center">
                    Choose your preferred format for downloading the anonymized CV
                  </p>
                </CardContent>
              </Card>
            )}
